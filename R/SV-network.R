# The main function to conduct network-assisted SV gene identification
# =========================================================================================
# The arguments are:
...
#  11) pattern: the specified spatialization pattern, where 'linear' is for Linear pattern, 'cos' is for Periodic pattern,
...
#  14) a: a specified value for the desired significant level, default to 0.05.
...
#  1) pars_ini: the list containing the initial values for the corresponding component.
#  2) save_mcmc:  the matrix containing the corresponding samples obtained after burn-in and thinning.
#  3) PIPs: the posterior inclusion probability vector of p genes.
#  4) loglik: the posterior model probability based on estimated parameters.
#  5) Pars: the list containing the estimated parameters for the corresponding component.
...
#  10) tune_set: the vector of length 2 with the first and second item representing the lower and upper bound of the uniform distribution 
#      for the threshold parameter respectively.
...
SV_network <- function(Y,B=NULL,W,net=NULL,nsim=2000,ntune=1000,freqTune=100,nest=1000,nthin=10,
                 pars_ini=NULL,pattern,q,loc,a=0.05,vrbs=T){
  ...
  if(!is.null(net)){
    ...
    laplacian1 <- laplacian2 <- L0
    mat1 <- sign(outer(beta1_ini, beta1_ini, "*"))
    mat2 <- sign(outer(beta2_ini, beta2_ini, "*"))
    ...
    subnets <- decompose.graph(net, mode = "weak")
  ...
  gamma1<-beta1_ini
  gamma2<-beta2_ini
  wt1<-foreach(t = vertices_List, .combine = "c") %do%{
    rep(min(sqrt(1.0/abs((gamma1[t])))),length(t))
  }
  
  wt2<-foreach(t = vertices_List, .combine = "c") %do%{
    rep(min(sqrt(1.0/abs((gamma2[t])))),length(t))
  }
  ...
  repeat{
    ...
    alpha_results<-update_alpha(R,Y,B,W,beta1, beta2,mu0,alpha,alpha.new,s,phi,
                                alphamean,alphasd,alpha.lower,alpha.upper,tau.alpha, accept.alpha)
    ...
    Ls<-deltaUGamma(R,Y,B,W,mu0,alpha,s,phi,beta1,beta2,gamma1,gamma2,lambda1,lambda2,1e-03)
    if(!is.null(net)){
      L_vec1<-as.vector(sigma.gamma1*eigmat1_inverse%*%Ls$d1)
      L_vec2<-as.vector(sigma.gamma2*eigmat2_inverse%*%Ls$d2)
      gamma1.new<-GenLangevin(gamma1,tau.gamma,sigma.gamma1,L_vec1,eigmat1_inv_List, vertices_List)
      gamma2.new<-GenLangevin(gamma2,tau.gamma,sigma.gamma2,L_vec2,eigmat2_inv_List, vertices_List)
    }else{
    ...
    gamma_results<-update_gamma(R,Y,B,W,beta1,beta2,gamma1,gamma2,gamma1.new,gamma2.new,
                                mu0,alpha,s,phi,lambda1,lambda2,accept.gamma)
    ...
    log_mh<-temp$log_mh+log(dtruncnorm(lbd,lambda.lower,lambda.upper,lbd_new, sqrt(tau.lambda))) -
  ...
  res<-list("pars_ini"=pars_ini,"save_mcmc"=save_mcmc,"PIPs"= PIPs,"loglik"=loglik,"Pars"=Pars,
            "B"=B,"count"=Y,"net"=net,"gene_ids"=sort(unique(ids)),"tune_set"=tune_set)
